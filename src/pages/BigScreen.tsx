...
import type { SemanticGroup, SemanticVariant } from '../lib/semanticGrouping';
...
export default function BigScreen() {
  ...
  const [groupedWordsSnapshot, setGroupedWordsSnapshot] = useState<SemanticGroup[]>([]);
    ...
    const groupedWordList = useMemo<
        { displayText: string; totalCount: number; variants: SemanticVariant[] }[]
    >(() => {
        if (!semanticGrouping) return [];
        return groupedWordsSnapshot
            .map((group) => ({
                displayText: group.displayText,
                totalCount: group.totalCount,
                variants: [...group.variants].sort((a, b) => b.count - a.count),
            }))
            .sort((a, b) => b.totalCount - a.totalCount);
    }, [groupedWordsSnapshot, semanticGrouping]);
        ...
        <>
                    ...
                    >
                        ...
                        <div className="w-full h-full">
                            <WordCloud
                                ...
                                onGroupsUpdate={setGroupedWordsSnapshot}
                            />
                        </div>
            ...
            {showWords && (
                <Modal
                    title={
                        semanticGrouping
                            ? `Grouped Words (${groupedWordList.length})`
                    ...
                    {semanticGrouping ? (
                        <div className="space-y-3">
                            <p className="text-xs text-gray-500">
                                Summarizing {entries.length} submissions after semantic grouping.
                            </p>
                            <ul className="space-y-2">
                                {groupedWordList.map((group) => (
                                    <li key={group.displayText} className="flex flex-col gap-1">
                                        <div className="flex items-center justify-between gap-2">
                                            <span className="font-chinese font-medium">{group.displayText}</span>
                                            <span className="text-sm text-gray-500">{group.totalCount}</span>
                                        </div>
                                        {group.variants.length > 1 && (
                                            <div className="space-y-1">
                                                {group.variants
                                                    .filter((variant) => variant.count >= 2)
                                                    .map((variant) => (
                                                        <div
                                                            key={`${group.displayText}-${variant.text}`}
                                                            className="flex items-center justify-between gap-2 pl-4 text-xs text-gray-500"
                                                        >
                                                            <span className="font-chinese">↳ {variant.text}</span>
                                                            <span>{variant.count}</span>
                                                        </div>
                                                    ))}
                                                {group.variants.some((variant) => variant.count < 2) && (
                        ...
                        <div>
                            ...
                            <div className="flex gap-2">
                                <button
                                    ...
                                    className={`flex-1 px-3 py-2 rounded-lg text-sm font-medium transition-colors ${displayMode === 'overview'
                                ...
                                <button
                                    ...
                                    className={`flex-1 px-3 py-2 rounded-lg text-sm font-medium transition-colors ${displayMode === 'balanced'
                                            ? 'bg-purple-600 text-white'
                                            : 'bg-gray-200 dark:bg-gray-700 hover:bg-gray-300 dark:hover:bg-gray-600'
                                        }`}
                                >
                                ...
                                <button
                                    ...
                                    className={`flex-1 px-3 py-2 rounded-lg text-sm font-medium transition-colors ${displayMode === 'detailed'
                                            ? 'bg-purple-600 text-white'
                                            : 'bg-gray-200 dark:bg-gray-700 hover:bg-gray-300 dark:hover:bg-gray-600'
                                        }`}
                                >
                        ...
                        <div>
                            ...
                            <div className="flex gap-2 mb-3">
                                <button
                                    ...
                                    className={`flex-1 px-3 py-2 rounded-lg text-sm font-medium transition-colors ${minOccMode === 'auto'
                                            ? 'bg-purple-600 text-white'
                                            : 'bg-gray-200 dark:bg-gray-700 hover:bg-gray-300 dark:hover:bg-gray-600'
                                        }`}
                                >
                                ...
                                <button
                                    ...
                                    className={`flex-1 px-3 py-2 rounded-lg text-sm font-medium transition-colors ${minOccMode === 'manual'
                                            ? 'bg-purple-600 text-white'
                                            : 'bg-gray-200 dark:bg-gray-700 hover:bg-gray-300 dark:hover:bg-gray-600'
                                        }`}
                                >
